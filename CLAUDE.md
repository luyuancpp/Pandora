...
| W4 ⑮ | 2026-06-12 | **team 新增 `GetMyTeam` RPC(第 8 个 RPC)+ `TeamBrief` 客户端简短视图**(解决"登录后客户端不知道自己有没有队伍/team_id"的入口;**不**塞进 login 返回,login 不耦合 team,队伍权威在 team 服务,客户端进大厅 UI 时调一次)。只读:player_id 以 JWT ctx 为准 → 查 `pandora:team:player:<id>` 索引 → 读 `TeamStorageRecord` 组装 `TeamBrief{team_id,state,captain_id,member_count,max_size}`(§5.11 最小视图,完整成员列表另调 GetTeam);**没队伍是正常态**返 OK+`has_team=false` 不用 errcode;索引命中但队伍记录已过期/已解散(TTL 竞态)按无队伍处理并顺手清脏索引(防 ClaimPlayer SETNX 残留挡住再建队,不变量 §1)。`[proto]` regen go+cpp pb 需同步 UE 仓库;biz 新增 4 单测;10 module BUILD=0,team VET/TEST=0。 |
